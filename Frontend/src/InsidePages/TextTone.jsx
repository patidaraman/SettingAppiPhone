...
function TextTone() {
  ...
  const [selectedOption, setSelectedOption] = useState("Radiate");

  const handleOptionClick = (option) => {
    setSelectedOption(option === selectedOption ? null : option);
    sendSelectedTexttoneToServer(option);
  };

  const sendSelectedTexttoneToServer = (selectedTexttone) => {
    fetch("http://localhost:8000/texttone/texttone", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ selectedTexttone }),
    })
      .then((response) => response.json())
      .then((data) => {
        console.log("Selected text tone sent to the server:", data);
        setSelectedOption(selectedTexttone); // Update the local state
      })
      .catch((error) => {
        ...
        <div className="Screen_View_Row">
          <div style={{ marginTop: "10px" }}>
            <span>
              {" "}
              <a
                href="https://apps.apple.com/il/app/ringtones-for-iphone-music/id380908399"
                style={{
                  textDecoration: "none",
                  color: "#007FFF",
                  fontSize: "17px",
