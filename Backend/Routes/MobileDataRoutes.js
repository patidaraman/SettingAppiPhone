...
mobiledataRouter.get("/IP", async (req, res) => {
  try {
    const existingRecord = await MobileDataModle.findOne();
    if (existingRecord) {
      res.json({ isLimitIPAddressTrackingOn: existingRecord.isLimitIPAddressTrackingOn });
    } else {
      res.status(404).json({ error: " Limit IP Address Tracking On state not found" });
    }
  } catch (error) {
    console.error('Error fetching Limit IP Address Tracking On   setting:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
mobiledataRouter.get("/other", async (req, res) => {
  try {
    const existingRecord = await MobileDataModle.findOne();
    if (existingRecord) {
      res.json({ isAllowCallsonOtherDevicesOn: existingRecord.isAllowCallsonOtherDevicesOn });
    } else {
      res.status(404).json({ error: " Allow Calls on Other DeviceO n state not found" });
    }
  } catch (error) {
    console.error('Error fetching Allow Calls on Other Devices On setting:', error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
mobiledataRouter.get("/mac", async (req, res) => {
  try {
    const existingRecord = await MobileDataModle.findOne();
    if (existingRecord) {
      res.json({ isAllowCallsOnMacBookOn: existingRecord.isAllowCallsOnMacBookOn });
    } else {
      res.status(404).json({ error: " Allow Calls On MacBook On state not found" });
    }
  } catch (error) {
